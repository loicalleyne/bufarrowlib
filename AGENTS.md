...
- **Do not expect `google.protobuf.Struct` / `Value` / `ListValue` to expand into nested Arrow structs.** They are mutually recursive and have no finite struct representation, so they terminate as a single Arrow `String` column holding the canonical protojson encoding (otel `AnyValue` terminates as `Binary`). Query them in DuckDB with the `json_*` functions. protojson output is **not byte-stable across processes** — compare semantically (`protojson.Unmarshal` then `proto.Equal`), never by string equality, and do not content-hash the column. An unset `google.protobuf.Value` cannot be marshalled at all and becomes a null; see [`docs/artifacts/protojson-wkt.md`](docs/artifacts/protojson-wkt.md).
- **Do not add a message type to the schema that recursively contains itself.** `New`/`NewFromFile` return `ErrCyclicType` naming the type and field path. Cycle detection is an ancestor-path set, so a type appearing in two sibling fields (a diamond) is fine; only a true cycle is rejected. `ErrMxDepth` remains as a secondary guard for deeply nested acyclic schemas.
- **Do not dispatch on message descriptor identity.** Descriptors compiled at runtime by protocompile (`NewFromFile`, pybufarrow) are distinct objects from linked-in generated ones, so `switch msg { case someDescriptor: }` silently misses. Compare `msg.FullName()` instead.
- **Do not assume the two output modes map well-known types the same way.** By default the full-fidelity path expands `Timestamp`, `Duration`, the `*Value` wrappers and `google.type.*` structurally, while the denormalizer maps them to flat Arrow scalars. `WithWellKnownTypes()` aligns the full path with the denormalizer; it is opt-in because it changes the Arrow and Parquet schema. `google.protobuf.Duration` is excluded even then — arrow-go's Parquet writer has no `DURATION` support, and `build()` converts the full-fidelity schema to Parquet at construction time.
